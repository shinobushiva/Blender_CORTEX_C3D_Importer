...
bl_info = {
    "name": "C3D Graphics Lab Motion Capture file from Cortex (.c3d)",
    "author": "Shinobu Izumi <stagesp1@gmail.com>",
    "version": (2017, 3, 23, 1),
    "blender": (2, 78, 0),
    "location": "File > Import",
    "description":  "Imports C3D Graphics Lab Motion Capture files which is exported from Cortex",
                    "This is based on C3D_Importer by Daniel Monteiro Basso <daniel@basso.inf.br>"
...
class C3DAnimateCloud(bpy.types.Operator):
    ...
    """
    bl_idname = "import_anim.c3dcortexanim"
    bl_label = "Animate C3D from Cortex"
...
class C3DImporter(bpy.types.Operator):
    ...
    """
    bl_idname = "import_anim_cortex.c3d"
    bl_label = "Import C3D (Cortex)"
    ...
    def execute(self, context):
        ...
        empty_size = (s, s, s)
        ms = import_c3d_cortex.read(self.properties.filepath, onlyHeader=True)
        ...
        bpy.ops.import_anim.c3dcortexanim()
        return {'FINISHED'}
...
def menu_func(self, context):
    self.layout.operator(C3DImporter.bl_idname,
                         text="Graphics Lab Motion Capture from Cortex (.c3d)")
